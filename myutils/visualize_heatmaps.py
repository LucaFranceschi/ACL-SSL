...
def plot_grid(root: str, img_filename: str, output_path: str, mode,
    ...
    for c, sample in enumerate(all_samples):
        ax = axes[0, c]
        short = sample if len(sample) <= 20 else sample[:18] + "…"
        # ax.set_title(short, fontsize=6, rotation=45, ha='left', va='bottom',
        #              pad=2, color="#333333")
    ...
    for r, exp in enumerate(exp_names):
        ...
        for c, sample in enumerate(all_samples):
            ...
            # Find audio from first available experiment for this sample
            audio_path = None
            for e in exp_names:
                bd = best_dirs.get(e)
                if bd:
